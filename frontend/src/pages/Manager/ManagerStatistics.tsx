  ...
  const handleReset = () => {
    setState((prevState) => ({
      ...
      series: prevState.series.map((serie) => ({
        ...serie,
        data: serie.data.map(() => Math.floor(Math.random() * 100)), // Example: randomize data
      })),
    }));
  };

  const handleFilter = (type: 'day' | 'week' | 'month') => {
    // Example: Implement data filtering logic based on type
    // This function should update the state's series based on the selected filter
    console.log(`Filtering data for ${type}`);
  };
